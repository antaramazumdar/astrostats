...
c
c     The output columns are: H,a,e,inc,diameter,albedo
c     
...
c     &     ,(alpha(ih,ia,ie,ii,j),j=1,nsour) ! source contributions
      model(ih,ia,ie,ii)=modelin      
c     fix boundaries..................... Fix 1
      if(ii.eq.1) then
         model(ih,ia,ie,0)=-modelin
      end if
      if(ie.eq.1) then
         model(ih,ia,0,ii)=-modelin
      end if
      if(ii.eq.1.and.ie.eq.1) then
         model(ih,ia,0,0)=-modelin
      end if
...
c      ld1=log10(0.003)
c      call splint(dpoint,nlogp,y2,npoint,ld1,ndiam1)
c      write(*,*)10.d0**ndiam1
c      stop
...
c     Trilinear interpolation for orbital elements
      ...
      if(ia0.lt.1)ia0=1
      if(ia1.gt.na)ia1=na
c      if(ie0.lt.1)ie0=1  ! Fix no. 1
      if(ie1.gt.ne)ie1=ne
c      if(ii0.lt.1)ii0=1  ! Fix no. 1
      if(ii1.gt.ni)ii1=ni
...
c     Step 1: trilinear for ih0
      ...
      if(wmod0.lt.0.d0) wmod0=0.d0 ! fix for NaN
...
c     Step 2: trilinear for ih1
      ...
      if(wmod1.lt.0.d0) wmod1=0.d0 ! fix for NaN
